...
char* indexes(const char* path)
{
    ...
    //if index.html exists, concatenate with index.html
    ...
    //if index.php exists, concatenate with index.php
...
const char* lookup(const char* path)
{
   // extract extension to token
    char pathcopy[strlen(path) + 1];
    strcpy(pathcopy, path);
    char* beginpathp = strtok(pathcopy, ".");
    char* extensionp = strtok(NULL, "\0");
    
    char extension[strlen(extensionp) + 1];
    strcpy(extension, extensionp);
    char beginpath[strlen(beginpathp) + 1];
    strcpy(beginpath, beginpathp);
           
    extension[strlen(extensionp)] = '\0';
    beginpath[strlen(beginpathp)] = '\0';
    
    // compare extension to return type
...
bool parse(const char* line, char* abs_path, char* query)
{
        ...
        //copy tokens into method, requesttarget and httpversion
        char method[strlen(methodcopy) + 1];
        strcpy(method, methodcopy);
        char requesttarget[strlen(requesttargetcopy) + 1];
        strcpy(requesttarget, requesttargetcopy);
        char httpversion[strlen(httpversioncopy) + 1];
        strcpy(httpversion, httpversioncopy);
     
        // add null terminators
        method[strlen(method)] = '\0';
        requesttarget[strlen(requesttarget)] = '\0';
        httpversion[strlen(httpversion)] = '\0';     
        ...
        char* abs_pathcopy;
        char* querycopy;
        abs_pathcopy = strtok(requesttarget, "?");
        querycopy = strtok(NULL, "\0");
    
        //if not null, copy to query
        if (querycopy != NULL)
        {
